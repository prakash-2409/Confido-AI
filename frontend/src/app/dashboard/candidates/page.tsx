...
export default function CandidatesPage() {
  ...
  if (isLoading) {
    return <LoadingState message="Loading candidates pipeline..." />;
  }

  if (isError) {
    return (
      <ErrorState 
        message={error instanceof Error ? error.message : "Failed to load candidates list."} 
        onRetry={refetch}
      />
    );
  }
      ...
      {filtered.length === 0 ? (
        <Card className="border-border">
          <EmptyState
            ...
            description={searchQuery ? `No candidates match "${searchQuery}".` : "Your recruiter pipeline is currently empty."}
            action={searchQuery ? {
              label: "Clear Search",
            ...
            <table className="w-full">
              ...
              <tbody>
                {filtered.map((candidate) => {
                  ...
                  return (
                    <tr key={candidate.id || candidate._id} className="border-b border-border/50 hover:bg-muted/20 transition-colors group">
                      {/* Candidate info */}
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <Link href={`/dashboard/candidates/${candidate.id || candidate._id}`} className="text-xs font-semibold hover:text-primary transition-colors">
                              {candidate.name}
                            </Link>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <EvidenceBadge level={evLevel as EvidenceLevel} size="sm" showIcon={false} />
                        </div>
                      </td>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-14 h-1.5 bg-muted rounded-full overflow-hidden">
                            <div
                              className={cn(
                                "h-full rounded-full",
                                candidate.hiringReadiness >= 70 ? 'bg-evidence-verified' : 
                                candidate.hiringReadiness >= 50 ? 'bg-evidence-review' : 'bg-evidence-risk'
                              )}
                      ...
                      <td className="px-4 py-3">
                        <span className="text-2xs text-muted-foreground">
                          {candidate.updatedAt ? new Date(candidate.updatedAt).toLocaleDateString() : 'Recent'}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link href={`/dashboard/candidates/${candidate.id || candidate._id}`}>
